...
void lua_utf8_register_libs(lua_State* L) {
	...
	lua_getglobal(L, LUA_STRLIBNAME);//luaL_register(L, LUA_STRLIBNAME, utf8_string_funcs);
	...
	lua_remove(L, -2);
	int i = lua_rawlen/*lua_objlen*/(L, -1);
	...
	int i = lua_rawlen(L, -1); // length of package.searchers (is 4)
	//stack: 'package','package.searchers'
	for (int k = 0; utf8_searchers[k] != NULL; k++) {
