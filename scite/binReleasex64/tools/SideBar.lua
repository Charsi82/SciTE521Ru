...
if not win then
	function MoveSideBarLeft()
		...
		SideBar_Hide() SideBar_Show()
		tabs:context_menu {
			'Move panel to right|MoveSideBarRight'
		}
	end
	...
	function MoveSideBarRight()
		...
		SideBar_Hide() SideBar_Show()
		tabs:context_menu {
			'Move panel to left|MoveSideBarLeft'
		}
	end

	if sidebar_position == 'right' then
		tabs:context_menu {
			'Move panel to left|MoveSideBarLeft',
		}
	else
...
list_dir:context_menu {
	...
	'Показать с текущим расширением|FileMan_MaskOnlyCurrentExt',
	'', -- separator
...
--[[-- restore bookmarks, 'gui' required
AddEventHandler("OnInit", function()
	local file = io.open(bm_file)
	if not file then return end
	for line in file:lines() do
		if line:match("%[%d+%]") then
			local bmk = {}
			bmk.BufferNumber = tonumber(file:read("*l"))
			bmk.LineText = file:read("*l")
			bmk.LineNumber = tonumber(file:read("*l"))
			bmk.FilePath = file:read("*l")
			table_bookmarks[#table_bookmarks+1] = bmk
		end
	end
	file:close()
...
-- store bookmarks
AddEventHandler("OnClosing", function()
	local file = io.open(bm_file,"wb")
	if file then
		for i, bmk in ipairs(table_bookmarks) do
			file:write("["..i.."]\n")
			file:write(bmk.BufferNumber.."\n")
			file:write(bmk.LineText.."\n")
			file:write(bmk.LineNumber.."\n")
			file:write(bmk.FilePath.."\n")
		end
		file:close()
	end
 end)]]
