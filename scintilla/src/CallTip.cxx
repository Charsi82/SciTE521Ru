...
void CallTip::WrapLine(const char* text, size_t offset, size_t length, SplitVector<size_t>& wrapPosList) {
	wrapPosList.DeleteAll();
	size_t lastWrapPos = -1;
	size_t nextWrapBound = offset + wrapBound;
...
PRectangle CallTip::PaintContents(Surface* surfaceWindow, bool draw) {
	...
	int ascent = static_cast<int>(std::round(surfaceWindow->Ascent(font.get()) - surfaceWindow->InternalLeading(font.get())));
	...
	int maxWidth = 0;
	int numLines = 0;
	SplitVector<size_t> wrapPosList;
	...
	while (moreChunks) {
		...
		size_t chunkOffset = static_cast<int>(chunkVal - val.c_str());
		size_t chunkLength = static_cast<int>(chunkEnd - chunkVal);
		size_t chunkEndOffset = chunkOffset + chunkLength;
		...
		int off = chunkOffset;
		do {
			size_t hlStart = chunkEndOffset;
			size_t hlEnd = chunkEndOffset;
			...
			int wrapPos = off;
			const std::string_view _chunkVal = chunkVal;
			for (i = 0; i < wrapPosList.Length(); i++) {
				if ((wrapPosList[i] >= off) && (wrapPosList[i] <= hlStart) && (wrapPos != wrapPosList[i])) {
					// line wrap is needed here
					DrawChunk(surfaceWindow, x, _chunkVal.substr(wrapPos - chunkOffset, wrapPosList[i] - chunkOffset),
			...
			DrawChunk(surfaceWindow, x, _chunkVal.substr(wrapPos - chunkOffset, hlStart - chunkOffset),
				ytext, rcClient, false, draw);
			...
			DrawChunk(surfaceWindow, x, _chunkVal.substr(wrapPos - chunkOffset, hlEnd - chunkOffset),
				ytext, rcClient, true, draw);
	...
	return PRectangle(0, 0, maxWidth + insetX, lineHeight * numLines - surfaceWindow->InternalLeading(font.get()) + 2 + 2);
}
//!-end-[BetterCalltips]
#else
...
//!-start-[BetterCalltips]
	...
	font = Font::Allocate(fp); // font.Create(fp);
	// Look for multiple lines in the text
	// Only support \n here - simply means container must avoid \r!
	rectUp = PRectangle(0, 0, 0, 0);
	rectDown = PRectangle(0, 0, 0, 0);
	offsetMain = insetX;            // changed to right edge of any arrows
	lineHeight = surfaceMeasure->Height(font.get());
	PRectangle rcSize = PaintContents(surfaceMeasure.get(), false);
...
void CallTip::AddHighlight(size_t start, size_t end) {
	if (start == end) return;
	size_t i = 0;
	while (i < startHighlight.Length()) {
		...
		if ((start <= endHighlight[i]) && (end >= startHighlight[i])) {
			...
			end = std::max(end, endHighlight[i]);
			// delete old range
